...
export default function Fromm() {
  ...
  // we cannot use async function but we can use a function inside useeffect which is async
  React.useEffect(()=>{
   async function getmanymemes(){
  ...
  // React.useEffect(()=>{
  //   fetch("https://api.imgflip.com/get_memes").then(res=>res.json()).then(data=>setAllMemesImages(data.data.memes))
...
// })
  ...
  function handelClick(){
    ...
    const ransNum=Math.floor(Math.random()* allMemesImages.length)
    setMeme((prevMeMe)=>{
     return {...prevMeMe, 
             url:allMemesImages[ransNum].url}
